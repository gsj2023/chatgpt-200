...
export function Settings() {
  ...
  const getExpire = () => {
    axios({
      method: "get",
      url: "https://test.abab2.cn/user/getexpire",
      withCredentials: true,
    }).then((res) => {
  ...
  const getCurrentNumber = () => {
    axios({
      method: "get",
      url: "https://test.abab2.cn/user/getCurrentNumber",
      withCredentials: true,
    }).then((res) => {
  ...
  const goLogin = () => {
    if (localStorage.getItem("userInfo")) {
      axios({
        method: "get",
        url: "https://test.abab2.cn/common/logout",
        withCredentials: true,
      }).then((res) => {
        if (res.data.code === 200) {
  ...
  function handleKeyDown(event: { key: string; }) {
    if (event.key === "Enter") {
      axios({
        method: "post",
        url: "https://test.abab2.cn/redemption/getRedem",
        data: {
          code,
    ...
    <ErrorBoundary>
      ...
      <div className={styles["settings"]}>
        <List>
          ...
          <ListItem title="gpt 4.0当前剩余对话次数(每三小时刷新)">
